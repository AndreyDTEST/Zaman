...
def test_budget_payment(
    ...
    allure.dynamic.title(f"Платёж {display_name}")

    kzt_accounts = [a for a in get_user_accounts if a["currency"] == "KZT"]
    assert kzt_accounts, f"Нет KZT-счёта для платежа {display_name}"
    payer_account = kzt_accounts[0]["iban"]

    period, month_ru, year = get_last_month_context()

    # Создание платежа
    with allure.step(f"1. Создать платёж: {display_name}"):
        ...
        assert resp.status_code == 200, f"Не создан платёж {display_name}"
        tx_data = resp.json()
    ...
    allure.attach(
        f"✅ Платёж {display_name} успешно в статусе IN_PROGRESS",
        name="Результат",
